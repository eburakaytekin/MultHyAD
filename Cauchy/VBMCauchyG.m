function [auc_ABMean,auc_ABMode,FARMean,FARMode,dtmapMean,dtmapMode]=VBMCauchyG(data,map,FiltOn,DAT)
...
%% Variational Bayes Method Parameter Estimation for MV Cauchy Block

[L,Sn,ExpM,ExpT,NN,PsiN,vN]=VBMCauchy(DataFiltered);
...
DetectionMean(idx)=((1+L)/2)*log(1+transpose(put'-m0)*ApproximationTforMean*(put'-m0));
DetectionMode(idx)=((1+L)/2)*log(1+transpose(put'-m0)*ApproximationTforMode*(put'-m0));
